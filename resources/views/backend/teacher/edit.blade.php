            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route('teacher.update',$teacher->id) }}">
                        ...
                        <div class="form-group row">
                            <label for="email" class="col-md-4 col-form-label text-md-right">{{ __('Course For') }}</label>
                            <select name="course" id="course" class="form-control" style="width:320px;">
                            @foreach($courses as $course)
                                <option value="{{ $course->id }}" {{ ($course->id == $teacher->course_id) ? 'selected' : "" }}> {{ $course->name }} </option>
                            @endforeach
                            </select>
                        </div>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-6">
                                <input id="phone" type="text" class="form-control" name="phone" value="{{ $teacher->phone }}" required>
                            </div>
                        </div>
